...
class CoachSkillResultFormPage extends StatefulWidget {
  final String eventId;
  final DateTime eventDate;
  final String playerId;
  final String playerName;
  final SkillResultModel? existing;

  const CoachSkillResultFormPage({
    super.key,
  ...
  @override
  State<CoachSkillResultFormPage> createState() =>
      _CoachSkillResultFormPageState();
}

class _CoachSkillResultFormPageState extends State<CoachSkillResultFormPage> {
  ...
  Future<List<MetricDefinitionModel>> _loadMetrics() async {
    final metrics = await _metricService.getByCategory(MetricCategory.skill);
  ...
  Future<void> _save(List<MetricDefinitionModel> definitions) async {
    ...
    final result = SkillResultModel(
      ...
      metrics: values,
      technicalScore: SkillResultModel.calculateScore(values, definitions),
    ...
    final ok = _isEdit
        ? await vm.updateSkillResult(widget.existing!.id, result)
        : await vm.saveSkillResult(result);
    if (!mounted) return;
    if (ok) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBarTitle(
        title: _isEdit ? 'Edit Skill' : 'Add Skill',
      ...
      body: FutureBuilder<List<MetricDefinitionModel>>(
        ...
        builder: (context, snapshot) {
          ...
          if (definitions.isEmpty) {
            return const Center(child: Text('No skill metrics'));
          }
          return Form(
            key: _formKey,
            child: ListView(
